...
public class AddEntryGUI extends JFrame {
    ...
    /** -1 means "add" mode; any valid index means "edit" mode. */
    private final int editIndex;
    /** The entry being edited (null in add mode). */
    private final JournalEntry editTarget;
    /** Optional callback fired after a successful save (e.g. to refresh a list). */
    private final Runnable onSaved;

    /** Standard constructor — creates the form in "add" mode. */
    public AddEntryGUI(DashboardGUI dashboard) {
        this(dashboard, -1, null, null);
    }

    /**
     * Edit-mode constructor — pre-fills the form with an existing entry.
     *
     * @param dashboard the dashboard to refresh (may be null)
     * @param editIndex index of the entry being edited within JournalManager
     * @param entry     the entry to pre-fill the form with
     * @param onSaved   callback fired after the edit is saved (may be null)
     */
    public AddEntryGUI(DashboardGUI dashboard, int editIndex,
                       JournalEntry entry, Runnable onSaved) {
        ...
        setTitle(editing ? "✏ Edit Journal Entry" : "✍ Add Journal Entry");
        setSize(720, 660);
        ...
        if (editing) prefillFromTarget();

        setVisible(true);
    }

    private boolean isEditMode() {
        return editTarget != null && editIndex >= 0;
    }

    /** Populate all input fields with the values from the entry being edited. */
    private void prefillFromTarget() {
        txtDate.setText(editTarget.getDate());
        txtTitle.setText(editTarget.getTitle());
        txtEntry.setText(editTarget.getEntry());
        moodBox.setSelectedItem(editTarget.getMood());
        // Keep the mood preview in sync with the pre-selected mood.
        String mood = (String) moodBox.getSelectedItem();
        moodPreview.setText(Theme.moodEmoji(mood));
        moodPreview.setBackground(Theme.moodColor(mood));
    }
    ...
    private JPanel buildHeader() {
        ...
        JLabel title = new JLabel(isEditMode()
            ? "Update your journal entry" : "How are you feeling today?");
    ...
    private JPanel buildForm() {
        ...
        JButton save = styledButton(
            isEditMode() ? "Update Entry" : "Save Entry", Theme.BUTTON);
        save.setIcon(isEditMode() ? IconFactory.update() : IconFactory.save());
    ...
    private void saveEntry() {
        try {
            ...
            if (isEditMode()) {
                JournalManager.getInstance().updateEntry(editIndex, je);
                FileHandler.saveAllJournal();

                JOptionPane.showMessageDialog(this,
                    "✅  Journal entry updated successfully!", "Updated",
                    JOptionPane.INFORMATION_MESSAGE);

                if (dashboard != null) dashboard.refreshDashboard();
                if (onSaved != null) onSaved.run();
                dispose();   // close the edit window once done
                return;
            }
