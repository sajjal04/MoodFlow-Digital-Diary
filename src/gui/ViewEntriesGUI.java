...
public class ViewEntriesGUI extends JFrame {
    ...
    private JPanel buildHeader() {
        ...
        searchBtn.setIcon(IconFactory.search());
    ...
    private RoundedPanel buildEntryCard(JournalEntry e, int idx) {
        ...
        // Edit button
        JButton edit = cardButton("Edit", Theme.BUTTON);
        edit.setIcon(IconFactory.edit());
        edit.addActionListener(ev -> openEditForm(idx));
        btnRow.add(edit);

        // Delete button
        JButton del = cardButton("Delete", Theme.BUTTON_DANGER);
        del.setIcon(IconFactory.delete());
    ...
    /** Build a small styled action button used inside entry cards. */
    private JButton cardButton(String text, Color bg) {
    ...
    /**
     * Open the pre-filled edit form for the entry at the given index.
     * After a successful save, the card list and dashboard are refreshed.
     */
    private void openEditForm(int idx) {
        JournalEntry target = JournalManager.getInstance().getEntry(idx);
        if (target == null) {
            JOptionPane.showMessageDialog(this,
                "This entry no longer exists.", "Edit Error",
                JOptionPane.WARNING_MESSAGE);
            refreshCards(searchField.getText().trim());
            return;
        }
        new AddEntryGUI(dashboard, idx, target,
            () -> refreshCards(searchField.getText().trim()));
    }
}
