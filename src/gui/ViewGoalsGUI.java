...
public class ViewGoalsGUI extends JFrame {
    ...
    private RoundedPanel buildGoalCard(Goal g, int idx) {
        ...
        if (!g.isCompleted()) {
            ...
            complete.setIcon(IconFactory.complete());
        ...
        JButton edit = actionBtn("Edit", Theme.BUTTON);
        edit.setIcon(IconFactory.edit());
        edit.addActionListener(e -> openEditForm(idx));
        btnRow.add(edit);

        JButton delete = actionBtn("Delete", Theme.BUTTON_DANGER);
        delete.setIcon(IconFactory.delete());
    ...
    /**
     * Open the pre-filled edit form for the goal at the given index.
     * After a successful save, the card list and dashboard are refreshed.
     */
    private void openEditForm(int idx) {
        Goal target = GoalManager.getInstance().getGoal(idx);
        if (target == null) {
            JOptionPane.showMessageDialog(this,
                "This goal no longer exists.", "Edit Error",
                JOptionPane.WARNING_MESSAGE);
            refreshCards();
            return;
        }
        new GoalGUI(dashboard, idx, target, this::refreshCards);
    }
