...
public class GoalGUI extends JFrame {
    ...
    /** The goal being edited (null in add mode). */
    private final Goal editTarget;
    ...
    /**
     * Edit-mode constructor — pre-fills the form with an existing goal.
     *
     * @param dashboard the dashboard to refresh (may be null)
     * @param editIndex index of the goal being edited within GoalManager
     * @param goal      the goal to pre-fill the form with
     * @param onSaved   callback fired after the edit is saved (may be null)
     */
    public GoalGUI(DashboardGUI dashboard, int editIndex,
                   Goal goal, Runnable onSaved) {
        this.dashboard  = dashboard;
        this.editIndex  = editIndex;
        this.editTarget = goal;
        this.onSaved    = onSaved;

        boolean editing = isEditMode();
        setTitle(editing ? "✏ Edit Goal" : " Add New Goal");
        setSize(700, 580);
    ...
    /** Populate all input fields with the values from the goal being edited. */
    private void prefillFromTarget() {
        txtGoal.setText(editTarget.getGoalName());
        categoryBox.setSelectedItem(editTarget.getCategory());
        String deadline = editTarget.getDeadline();
        txtDeadline.setText(deadline.isEmpty() ? "(optional)" : deadline);
        progressSlider.setValue(editTarget.getProgress());
        progressLabel.setText(editTarget.getProgress() + "%");
    }
    ...
    private JPanel buildHeader() {
        ...
        JLabel lbl = new JLabel(isEditMode() ? "Update your Goal ️" : "Set a New Goal ");
    ...
    private JPanel buildForm() {
        ...
        // Save button
        g.gridx = 1; g.gridy = 4;
        JButton save = btn(isEditMode() ? "Update Goal" : "Save Goal", Theme.BUTTON);
    ...
    private void saveGoal() {
        try {
            ...
            if (isEditMode()) {
                GoalManager.getInstance().updateGoal(editIndex, goal);
                GoalFileHandler.saveAllGoals();

                JOptionPane.showMessageDialog(this,
                    "  Goal updated successfully!", "Updated",
