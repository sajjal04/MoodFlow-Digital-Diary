...
public class GoalManager {
    ...
    /** Replace the goal at the given index with an updated goal. */
    public void updateGoal(int index, Goal goal) {
        if (goal != null && index >= 0 && index < goals.size()) {
            goals.set(index, goal);
        }
    }

    /** Safe accessor for a single goal, or null when out of range. */
